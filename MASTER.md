...
- **Backend:** FastAPI (**Python 3.12**) + Redis _(Kaden, Inseon)_. Runs as **3 processes** (or 2 with
...
- **AI:** Claude for extraction / target-parse / lesson-gen / 2AFC judging (**not** the deterministic
  score — see §7). A **larger Claude model** is used once per session to draft the judging rubric
  consumed by the (likely smaller/faster) judge calls — see §5.2 **[OPEN]**.
- **Agents (Fetch.ai uAgents):** `resource_agent` (curated free FGLI resources per gap) and
  ...
  standalone uAgents; FastAPI bridges via `send_sync_message()`.
...
### API contract — authoritative spec in **`API_CONTRACT.md`** (FE/BE source of truth)
> The table below is the simplified mental model. **`API_CONTRACT.md`** is authoritative and supersedes
> it: `/score` is now a **multipart file upload** (`user_id`, `resume_file`, `target: {type, value}`),
> the response carries a `status` object (per-section complete/pending/failed), and all endpoints share
> one error shape. Align backend code to `API_CONTRACT.md`.
